...
export const studioGeneralScene = {
  ...
  editorIds: ['identity', 'event-canonical', 'event-operations'],
} as const
...
export function selectSceneAfterExclusion(sceneId: StudioSceneId, draft: Pick<Origin01StudioDraft, 'modules'>): StudioSceneId {
  ...
  const next = studioPublicScenes.slice(excludedIndex + 1).find((scene) => isStudioSceneIncluded(draft, scene))
  if (next) return next.id
  const previous = studioPublicScenes.slice(0, excludedIndex).reverse().find((scene) => isStudioSceneIncluded(draft, scene))
