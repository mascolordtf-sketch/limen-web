...
Studio conserva su interfaz y su renderer público, pero separa el borrador temporal de la composición de página. `useOrigin01StudioModel` es la frontera local para inicialización, actualizaciones tipadas, activación protegida de escenas y restablecimientos; las funciones puras del mismo módulo derivan la invitación de preview y calculan validación jerárquica sin mutar el fixture. El borrador mantiene las fuentes canónicas y el contenido editorial, mientras las etiquetas, firmas, mensajes identitarios y demás proyecciones se producen únicamente al derivar la invitación entregada a Origin 01.

Los contratos compartidos de Studio definen los cinco dominios y una intención futura de preview contextual. La composición secundaria, el orden narrativo, las escenas y el esquema preparatorio de Trivia permanecen en `origin01StudioConfiguration`, porque pertenecen a la plantilla y no son verdades universales del shell. La audiencia continúa fuera del contenido del borrador y cambiarla reinicia el renderer.

La validación distingue estructura renderizable, errores editoriales activos, advertencias, contenido inválido de escenas inactivas y revisión editorial pendiente. Una escena opcional inactiva conserva sus datos y deja de aportar errores relevantes; una operación de modelo rechaza la desactivación de escenas obligatorias. `Invitación válida`, revisión editorial y la capacidad futura `Lista para publicar` permanecen estados diferentes. No existen todavía persistencia ni una última versión guardada: la frontera de dirty state expone solamente borrador inicial y actual.
